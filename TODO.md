...
- [x] **Step 2**: Create tool module
  ...
  - **BashCommand tool**: accepts `command`, validates first word against whitelist (`grep`, `find`, `cat`, `head`, `tail`, `ls`, `tree`, `wc`, `file`, `rg`), allows pipes `|` for chaining commands, rejects semicolons `;`, `&&`, `||`, backticks, `$()`, redirects `>` `<`, executes with cwd set to base dir, kills after 30 seconds using `tokio::time::timeout`
  - **Note**: Pipe support added to allow common command chains like `find ... | head -20`
