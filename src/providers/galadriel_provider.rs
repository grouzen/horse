...
impl Provider for GaladrielProvider {
    fn name(&self) -> &str {
        "galadriel"
    }

    fn required_env_vars(&self) -> Vec<&str> {
        vec!["GALADRIEL_API_KEY"]
    }
    ...
    fn create_agent(
        ...
        Ok(Box::new(GaladrielAgentWrapper { agent }))
    }
}

struct GaladrielAgentWrapper {
    agent: Agent<galadriel::CompletionModel>,
}

#[async_trait::async_trait]
impl AgentWrapper for GaladrielAgentWrapper {
    async fn prompt(
        &mut self,
        input: String,
